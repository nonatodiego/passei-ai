...
export const TodayService = {
  async getToday(): Promise<TodayData> {
    ...
    const [sessions, blocks, reviews, errors, goals, decision] = await Promise.all([
      db.studySessions.toArray(), db.questionBlocks.toArray(), db.reviews.toArray(), db.errorRecords.toArray(), db.goals.toArray(), getLocalDecisionEngineResult(),
    ]);
    ...
    return {
      ...
      preparationMessage: decision.dataSufficiency.message ?? 'Indice em formacao com seus dados locais.', preparationTrend: '', weeklyStudiedHours: Math.round(decision.summary.weeklyMinutes / 6) / 10, weeklyGoalHours: getGoalTarget(goals, 'hours', 15),
      ...
      recommendations: decision.recommendations.map((item) => ({ id: item.id, title: item.title, problem: item.problem, evidence: item.evidence.map((e) => `${e.label}: ${e.value}`).join(' - '), impact: item.impact, estimatedMinutes: item.estimatedMinutes, actionLabel: item.actionLabel, actionRoute: item.actionRoute, urgency: item.urgency === 'critical' || item.urgency === 'high' ? 'high' as const : item.urgency === 'medium' ? 'medium' as const : 'low' as const })),
