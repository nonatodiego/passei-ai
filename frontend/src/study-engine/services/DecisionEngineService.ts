...
import { getGoalTarget } from '@/goals/services';
import {
  addLocalDays,
...
const daysBetween = (from: string, to: string) => Math.max(0, differenceInLocalDays(from, to));
const startOfWeek = (date: string) => getLocalWeekRange(parseLocalDateKey(date)).start;
const recommendation = (value: StudyRecommendation): StudyRecommendation => value;
const materialAliases: Record<string, string> = { videoaula: 'video', questoes: 'questions', revisao: 'review', simulado: 'mockExam', leitura: 'reading' };
const estimatedScheduleMinutes = (item: ScheduleItem, estimates: Record<string, number>) => {
  if (typeof item.actualMinutes === 'number' && item.actualMinutes > 0) return item.actualMinutes;
  const normalizedType = item.activityType.normalize('NFD').replace(/[\u0300-\u036f]/g, '').toLowerCase();
  return estimates[materialAliases[normalizedType] ?? normalizedType] ?? 45;
};

export function generateDecisionEngineResult(input: DecisionEngineInput): DecisionEngineResult {
  const today = toLocalDateKeyFromValue(input.now);
  ...
  const remainingDays = Math.max(1, 7 - (parseLocalDateKey(today)?.getDay() || 7)); const remainingMinutes = Math.max(0, input.weeklyGoalMinutes - weeklyMinutes);
  ...
  if (studiedDisciplines.size >= 2) {
    const cutoff = addLocalDays(today, -14);
    ...
    Object.entries(lastByDiscipline).filter(([, date]) => date < cutoff).forEach(([disciplineName, date]) => {
  ...
  const overdueItems = Object.values(overdueByDiscipline).flat(); if (overdueItems.length >= 5) recommendations.push(recommendation({ id: 'schedule-backlog', ruleId: 'schedule-backlog', title: 'Reduza o acumulo do cronograma', problem: 'Carga atrasada relevante', reason: 'Ha varias atividades atrasadas antes da data da prova.', evidence: [{ label: 'Atividades atrasadas', value: String(overdueItems.length) }, { label: 'Carga estimada', value: `${overdueItems.reduce((sum, item) => sum + estimatedScheduleMinutes(item, input.materialEstimates), 0)} min` }], impact: 'Uma recuperacao gradual melhora a visibilidade do plano.', actionLabel: 'Ver cronograma', actionRoute: '/cronograma', urgency: 'high', estimatedMinutes: 45 }));
